...
import { EnemyType, PowerUpType, type Alien, type Bullet, type PowerUp, type Explosion, type Particle, type Star, type ActivePowerUps, type LevelConfig } from '@/lib/types';
...
export default function SpaceShooterGame(props: SpaceShooterGameProps) {
  ...
  // Level configuration - 10 unique levels with different enemy appearances
  const getLevelConfig = (level: number): LevelConfig => {
    const configs: LevelConfig[] = [
      ...
      { 
        ...
        enemyColor: '#84cc16',
        enemyAccentColor: '#65a30d',
        enemyGlowColor: '#bef264'
      },
      { 
        level: 4, 
        enemies: 30, 
        ...
        spawnDelay: 1400, 
        description: "Serpent Strike",
        enemyColor: '#06b6d4',
        enemyAccentColor: '#0891b2',
        enemyGlowColor: '#67e8f9'
      },
      { 
        level: 5, 
        enemies: 35, 
        ...
        spawnDelay: 1200, 
        description: "Gunner Squad",
        enemyColor: '#8b5cf6',
        enemyAccentColor: '#7c3aed',
        enemyGlowColor: '#c4b5fd'
      },
      { 
        level: 6, 
        enemies: 40, 
      ...
      { 
        ...
        enemyColor: '#eab308',
        enemyAccentColor: '#ca8a04',
        enemyGlowColor: '#fde047'
      },
      { 
        level: 8, 
        enemies: 50, 
        ...
        enemyColor: '#14b8a6',
        enemyAccentColor: '#0d9488',
        enemyGlowColor: '#5eead4'
      },
      { 
        level: 9, 
        enemies: 60, 
        ...
        enemyColor: '#f43f5e',
        enemyAccentColor: '#e11d48',
        enemyGlowColor: '#fda4af'
      },
      { 
        level: 10, 
        enemies: 1, 
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      // Spawn new aliens based on level configuration
      const now = Date.now();
      const levelConfig = getLevelConfig(level);
      
      // Only spawn if we haven't reached the level's enemy limit
      if (enemiesSpawnedInLevel < levelConfig.enemies && now - lastAlienSpawnRef.current > levelConfig.spawnDelay) {
        ...
        const newAlien: Alien = {
          ...
          health: type === EnemyType.BOSS ? 30 + (level * 5) : stats.health,
          maxHealth: type === EnemyType.BOSS ? 30 + (level * 5) : stats.health,
      ...
      if (enemiesKilledInLevel >= levelConfig.enemies && currentAliens.length === 0 && enemiesSpawnedInLevel >= levelConfig.enemies) {
  ...
  return (
    ...
    >
        ...
        <div className="relative flex justify-between items-center">
          ...
          <div className="flex gap-4 items-center">
            ...
            <div className="text-purple-400 font-mono text-sm">
              {enemiesKilledInLevel}/{getLevelConfig(level).enemies}
            </div>
            ...
            <svg viewBox="0 0 50 50" className="w-full h-full">
              <defs>
                <linearGradient id={`alienGradient${alien.id}`} x1="0%" y1="0%" x2="0%" y2="100%">
                  <stop offset="0%" stopColor={levelConfig.enemyColor} />
                  <stop offset="50%" stopColor={levelConfig.enemyAccentColor} />
                  <stop offset="100%" stopColor={levelConfig.enemyAccentColor} />
                </linearGradient>
              </defs>
              ...
              <path
                ...
                stroke={levelConfig.enemyAccentColor}
                strokeWidth="2"
              />
              {/* Left wing */}
              <path
                d="M10 20 L5 25 L10 28 Z"
                fill={levelConfig.enemyAccentColor}
                stroke={levelConfig.enemyAccentColor}
                strokeWidth="1.5"
              />
              {/* Right wing */}
              ...
              <circle cx="25" cy="18" r="3" fill={levelConfig.enemyColor} opacity="0.7">
                <animate attributeName="opacity" values="0.5;0.9;0.5" dur="1s" repeatCount="indefinite" />
              </circle>
              {/* Engine exhausts */}
              <circle cx="18" cy="32" r="2" fill={levelConfig.enemyGlowColor} opacity="0.8">
                <animate attributeName="opacity" values="0.6;1;0.6" dur="0.3s" repeatCount="indefinite" />
              </circle>
              <circle cx="32" cy="32" r="2" fill={levelConfig.enemyGlowColor} opacity="0.8">
                <animate attributeName="opacity" values="0.6;1;0.6" dur="0.3s" repeatCount="indefinite" />
              </circle>
            </svg>
          </div>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center z-20 px-4 overflow-y-auto py-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-cyan-400 font-mono text-sm">
              10 LEVELS • EPIC BOSS BATTLE
            </p>
          </div>

          {/* Show Leaderboard or Instructions */}
          {showLeaderboard ? (
            <div className="mb-8">
              <Leaderboard />
            </div>
      ...
      {/* Name Input Modal */}
      {showNameInput && <NameInput onSubmit={handleNameSubmit} onCancel={handleNameCancel} />}

      {/* Game Over Screen - Retro Style */}
      {gameOver && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-20 px-4 overflow-y-auto py-8">
          <div className="retro-box border-4 border-red-500 bg-black p-12 max-w-2xl mb-8">
            <h2 className={`text-6xl font-bold mb-6 text-center ${gameWon ? 'text-yellow-400' : 'text-red-500'}`} style={{
              fontFamily: 'monospace',
              letterSpacing: '0.2em'
            }}>
            ...
            {gameWon && (
              <p className="text-center text-cyan-400 font-mono text-lg mb-6">
                YOU DEFEATED ALL {CONSTANTS.TOTAL_LEVELS} LEVELS!
              </p>
            )}
            
            <div className={`border-4 ${gameWon ? 'border-yellow-500/50' : 'border-red-500/50'} bg-black p-6 mb-4`}>
              ...
              {playerName && (
                <div className="text-yellow-400 text-center text-sm mt-2" style={{ fontFamily: 'monospace' }}>
                  PLAYER: {playerName}
                </div>
              )}
            </div>
            ...
            <div className="flex gap-4">
              <button
                onClick={handleStartGameClick}
                className="flex-1 retro-button px-8 py-4 border-4 border-green-400 bg-black text-green-400 font-bold text-xl hover:bg-green-400 hover:text-black transition-all"
                style={{
                  fontFamily: 'monospace',
                  letterSpacing: '0.15em'
                }}
              ...
              <button
                onClick={handleShowLeaderboard}
                className="flex-1 retro-button px-8 py-4 border-4 border-yellow-400 bg-black text-yellow-400 font-bold text-xl hover:bg-yellow-400 hover:text-black transition-all"
                style={{
                  fontFamily: 'monospace',
                  letterSpacing: '0.15em'
                }}
        ...
        <div className="absolute inset-0 flex items-center justify-center z-30 bg-black/80">
          <div className="retro-box border-4 border-cyan-400 bg-black p-12 max-w-lg">
            ...
            <div className="border-4 border-cyan-400/50 bg-black p-6 mb-8">
              ...
              <div className="text-3xl font-bold text-green-400 text-center mb-4" style={{ fontFamily: 'monospace' }}>
                SCORE: {score.toString().padStart(6, '0')}
              </div>
              <div className="text-yellow-400 text-center text-lg" style={{ fontFamily: 'monospace' }}>
                NEXT: LEVEL {level + 1}
              </div>
              <div className="text-cyan-400 text-center text-sm mt-2" style={{ fontFamily: 'monospace' }}>
                {getLevelConfig(level + 1).description}
              </div>
            </div>

            <button
              onClick={handleNextLevel}
              className="w-full retro-button px-8 py-4 border-4 border-green-400 bg-black text-green-400 font-bold text-2xl hover:bg-green-400 hover:text-black transition-all"
              style={{
                fontFamily: 'monospace',
                letterSpacing: '0.15em'
              }}
